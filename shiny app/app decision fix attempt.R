...
tides <- function(mean, sd, n, min, max, is_interval = TRUE, verbose = TRUE){
...
plot_tides <- function(mean, sd, n, min, max, is_interval){
  dat <- tibble(mean = mean, 
                sd = sd,
                n = n,
                min = min,
                max = max) |>
    mutate(results = pmap(list(mean, sd, n, min, max, is_interval, verbose = FALSE), tides)) |>
    unnest(results) 
  ...
  data_plot <- 
    expand_grid(min = min,
                ...
                n = n) |>
    mutate(results = pmap(list(plotting_mean, sd, n, min, max, is_interval, verbose = FALSE), tides)) |>
    unnest(results) |>
...
plot_tides_with_min_sd <- function(mean, sd, n, min, max, is_interval){
  dat <- tibble(mean = mean, 
                sd = sd,
                n = n,
                min = min,
...
server <- function(input, output) {
  ...
  output$result_plot <- renderPlot({
    if (input$scale_is_interval) {
      plot_tides_with_min_sd(mean = input$mean, sd = input$sd, n = input$n, min = input$min, max = input$max, is_interval = input$scale_is_interval)
    } else {
      plot_tides(mean = input$mean, sd = input$sd, n = input$n, min = input$min, max = input$max, is_interval = input$scale_is_interval)
    }
  })
