...
p_grim_n_11_with_colors <- dat_n_11 |>
  filter(grim) |>
...
p_grimmer_n_11_with_colors <- dat_n_11 |>
  filter(grim & grimmer) |>
...
p_tides_n_11_with_colors <- dat_n_11 |>
  filter(grim & grimmer & tides) |>
...
p_grim_n_11_with_colors + 
  p_grimmer_n_11_with_colors + 
  p_tides_n_11_with_colors + 
  plot_layout(ncol = 1)
...
dat_n_11_combined <- 
  ...
  mutate(max_sd_new_rounded = round(max_sd_new, 2),
         ...
         min_sd_match = case_when(min_sd_.sd_limits == min_sd_new_rounded |
                                    (is.na(min_sd_.sd_limits) &
                                       is.na(min_sd_new_rounded)) ~
                                    TRUE,
                                  TRUE ~ FALSE),
         max_sd_match = case_when(max_sd_.sd_limits == max_sd_new_rounded |
                                    (is.na(max_sd_.sd_limits) &
                                       is.na(max_sd_new_rounded)) ~
                                    TRUE,
                                  TRUE ~ FALSE))
...
temp <- dat_n_11_combined |>
  ...
  distinct(min_sd_.sd_limits, 
           ...
           #max_sd_match, 
           grim.x, grimmer.x, 
           tides_modified_from_.sd_limits, tides_new,
           .keep_all = TRUE) |>
...
- view temp for examples of impossible scores
...
.sd_limits(n_obs = 11, mean = 1.27, min_val = 1, max_val = 7, sd_prec = 2, n_items = 1)

tides_modified_from_.sd_limits(n_obs = 11, mean = 1.27, sd = 0.00, min_val = 1, max_val = 7, sd_prec = 2, n_items = 1)
