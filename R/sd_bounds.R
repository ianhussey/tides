...
#' @examples
#' \dontrun{
#' # Suppose a 5-point scale (1–5), mean = 3.2, n = 30, no within-person averaging:
...
sd_bounds <- function(
  ...
  for (sc in scenarios) {
    ...
    if (
      janitor::round_half_up(mean(vec), digits) ==
        janitor::round_half_up(mean, digits) &&
  ...
  result_df <- data.frame(min_sd = result[1], max_sd = result[2])
