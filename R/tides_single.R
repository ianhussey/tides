...
tides_single <- function(mean, sd, n, min, max, n_items = 1, digits = NULL,
  ...
  # when SD is zero, this divides by zero and returns NaN. Replace these with 0 as a dummy value so that they are plotted rather than ignored.
  if (is.nan(pomp_sd)) { pomp_sd <- 0 }
